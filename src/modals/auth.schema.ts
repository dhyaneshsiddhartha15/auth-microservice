...
import { IAuthDocument } from "@dhyaneshsiddhartha15/jobber-shared";
...
const AuthModel: ModelDefined<IAuthDocument, AuthUserCreationAttributes>  & AuthModelInstanceMethods= sequelize.define('auths', {
  ...
  AuthModel.addHook('beforeCreate',async (auth:Model)=>{
   const hashPssword:string=await hash(auth.dataValues.password as string,SALT_ROUND);
   auth.dataValues.password=hashPssword;
   console.log("the has  password is " + auth.dataValues.password)
   console.log("the has  password is " + hashPssword)
  });
